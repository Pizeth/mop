	...
	<article id="post-<?php the_ID(); ?>">
		...
		<header class="entry-header">
			...
			<h2 class="entry-title">
				<a href="<?php the_permalink(); ?>" rel="bookmark" title="<?php echo esc_attr( sprintf( __( 'Permalink to %s', 'iconic-one' ), the_title_attribute( 'echo=0' ) ) ); ?>">
					<?php the_title(); ?>
				</a>
			</h2>
			<?php endif; // is_single() ?>
			...
			<div class="below-title-meta">
				...
				<div class="adt-comment">
					<a class="link-comments" href=
					"<?php comments_link(); ?>"><?php comments_number(__('0 មតិ','iconic-one'),__('1 មតិ','iconic-one'),__('% មតិ','iconic-one')); ?></a>
				</div>
			</div>
		...
		<?php endif; ?>
		<footer class="entry-meta <?php echo more_posts() == 0 ? 'last-post':''; ?>">
			<?php if ( is_home() && ( get_theme_mod( 'iconic_one_catg_home' , '1' ) == '1' ) ) : ?>
				<span>
					<?php _e('ប្រភេទអត្ថបទ :','iconic-one'); ?>
					<?php the_category(' '); ?>
				</span>
				<!-- <span class="meta-sep">|</span> 
				<span class="date updated">
			...
			<?php elseif( is_category() ): ?>
				...
				<div class="below-title-meta">
					...
					<!-- <div class="adt-comment">
						<a class="link-comments" href=
						"<?php comments_link(); ?>"><?php comments_number(__('0 Comment','iconic-one'),__('1 Comment','iconic-one'),__('% Comments','iconic-one')); ?></a>
					</div> -->
				</div>	
			<?php endif; ?>
			...
			<div class="author-info">
				...
				<div class="author-description">
					<h2>
					<?php printf( __( 'About %s', 'iconic-one' ), get_the_author() ); ?></h2>
					<p><?php the_author_meta( 'description' ); ?></p>
					<div class="author-link">
						<a href=
						"%3C?php%20echo%20esc_url(%20get_author_posts_url(%20get_the_author_meta(%20'ID'%20)%20)%20);%20?%3E"
						rel=
						"author"><?php printf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>', 'iconic-one' ), get_the_author() ); ?></a>
					</div><!-- .author-link -->
				</div><!-- .author-description -->
			</div><!-- .author-info -->
			<?php endif; ?>
		</footer><!-- .entry-meta -->
		<?php echo author_info_box(); ?>
	</article><!-- #post -->